...
void deallocate_customer(int customer_idx) {
    ...
    if (c->state == CUSTOMER_STATE_RESTING) {
        ...
        // Remove from heap if in waiting queue
        if (GLB[customer_idx].heap_index != NIL) {
            heap_delete(customer_idx);
        }
        shm->waiting_count--;
    }
    // USING state handled elsewhere (should not happen during dealloc)
...
void assign_tool_to_customer(int customer_idx, int tool_id) {
    ...
    // FIX: Check state instead of heap_index for waiting removal
    // Remove from waiting queue if customer was waiting
...
void handle_request(int customer_idx, int duration) {
    ...
    // FIX: Properly handle state transition from RESTING
    if (c->state == CUSTOMER_STATE_RESTING) {
        shm->resting_customers--;
    } else if (c->state == CUSTOMER_STATE_WAITING) {
        // Customer was already waiting, remove from queue
        if (GLB[customer_idx].heap_index != NIL) {
            heap_delete(customer_idx);
        }
        shm->waiting_count--;
...
void handle_rest(int customer_idx) {
    ...
    if (c->state == CUSTOMER_STATE_USING) {
        // Save tool ID before removing customer
...
void handle_report(int socket_fd) {
    ...
    // Check all customers for waiting state
    for (int i = 0; i < MAX_CUSTOMERS; i++) {
...
void agent_process(int client_socket) {
    ...
    // If customer still using tool, free it
